...
from app.models import ShoppingItem, ShoppingList, User, ItemPrice, Store, Chain
...
class PredictionService:
    ...
    def get_predictions(
        ...
        frequent_items = self._get_frequently_bought_items(
            user.id, household_ids, existing_items
        )
        predictions.extend(frequent_items[: limit // 2])

        # 2. Household favorites
        household_favorites = self._get_household_favorites(
            household_ids, user.id, existing_items
        )
        predictions.extend(household_favorites[: limit // 3])

        # 3. Recently purchased that might need replenishment
        replenishment_items = self._get_replenishment_predictions(
            user.id, household_ids, existing_items
        )
    ...
    def _get_frequently_bought_items(
        self, user_id: int, household_ids: List[int], existing_items: set
    ) -> List[ItemPrediction]:
        ...
        frequent_items = (
            ...
            .group_by(ShoppingItem.name, ShoppingItem.item_code)
            .having(
    ...
    def _get_household_favorites(
        self, household_ids: List[int], user_id: int, existing_items: set
    ) -> List[ItemPrediction]:
        ...
        household_items = (
            self.db.query(
                ...
                func.count(ShoppingItem.id).label("purchase_count"),
                func.avg(ShoppingItem.quantity).label("avg_quantity"),
                func.count(func.distinct(ShoppingItem.purchased_by_id)).label(
                    "unique_buyers"
                ),
            )
            .join(ShoppingList)
            ...
            .group_by(ShoppingItem.name, ShoppingItem.item_code)
            .having(
                func.count(func.distinct(ShoppingItem.purchased_by_id))
    ...
    def _get_replenishment_predictions(
        ...
        for item in regular_items:
            if item.name.lower() not in existing_items and item.purchase_count > 2:
                ...
                purchase_dates = (
                    self.db.query(ShoppingItem.purchased_at)
                    .join(ShoppingList)
                    .filter(
                        and_(
                            ShoppingList.household_id.in_(household_ids),
                ...
                if len(purchase_dates) >= 2:
                    ...
                    for i in range(len(purchase_dates) - 1):
                        interval = (
                            purchase_dates[i][0] - purchase_dates[i + 1][0]
                        ).days
                        if interval > 0:
                    ...
                    if intervals:
                        ...
                        if days_since_last >= avg_interval * 0.8:
                            confidence = min(
                                0.8, 0.5 + (days_since_last / avg_interval) * 0.3
                            )
