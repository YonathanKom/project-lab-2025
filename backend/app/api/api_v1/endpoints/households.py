...
from app.models.models import User, Household, user_households, HouseholdInvitation
...
def is_household_admin(db: Session, user_id: int, household_id: int) -> bool:
    ...
    user_role = db.execute(
        text("SELECT role FROM user_households WHERE user_id = :user_id AND household_id = :household_id"),
...
def invite_to_household(
    ...
    existing_invite = db.query(HouseholdInvitation).filter(
...
def get_sent_invitations(
    ...
    invitations = db.query(HouseholdInvitation).filter(
        HouseholdInvitation.invited_by_id == current_user.id
    ).all()
